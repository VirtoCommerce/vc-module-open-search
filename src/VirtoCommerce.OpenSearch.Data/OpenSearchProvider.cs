...
using IndexingSettings = VirtoCommerce.OpenSearch.Data.ModuleConstants.Settings.Indexing;
...
namespace VirtoCommerce.OpenSearch.Data
{
    public class OpenSearchProvider : ISearchProvider, ISupportIndexSwap, ISupportPartialUpdate, ISupportSuggestions, ISupportIndexCreate
    {
        ...
        protected OpenSearchRequestBuilder RequestBuilder { get; }
        ...
        public void AddActiveAlias(IEnumerable<string> documentTypes)
        {
            ...
            {
                _logger.LogError(ex, $"Error while putting an active alias on a default index at {nameof(AddActiveAlias)}. Possible fail on OpenSearch server side at IndexExists check.");
            }
        }
        ...
        public virtual async Task<SuggestionResponse> GetSuggestionsAsync(string documentType, SuggestionRequest request)
        {
            ...
            try
            {
                var suggestRequest = new OpenSearchRequest(indexName)
                {
                    Source = false,
        ...
        protected virtual SearchDocument ConvertToProviderDocument(IndexDocument document, IProperties properties)
        {
            ...
            foreach (var field in document.Fields.OrderBy(f => f.Name))
            {
                ...
                else
                {
                    ...
                    {
                        value = isCollection
                            ? field.Values.Select(v => ((GeoPoint)v).ToOpenSearchValue()).ToArray()
                            : point.ToOpenSearchValue();
                    }
                    else
                    {
                        value = isCollection
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        {
            // https://opensearch.org/docs/latest/field-types/#mapping-limit-settings
            var fieldsLimit = GetFieldsLimit();

            //https://opensearch.org/docs/latest/install-and-configure/configuring-opensearch/index-settings/#dynamic-index-level-index-settings
